...
bool cTcpServer::AcceptProcess()
{
	...
	const int ret = select(maxfd, &acceptSockets, NULL, NULL, &t);
	if (0 == ret)
		return true; // nothing~
	if (SOCKET_ERROR == ret)
	...
	sockaddr_in addr;
	SOCKET remoteSocket = accept(m_socket, (struct sockaddr*)&addr, &clen);
	...
	getpeername(remoteSocket, (struct sockaddr*)&addr, &clen);
...
bool cTcpServer::ReceiveProcces()
{
	...
	struct timeval t = { 0, m_sessions.empty() ? 10000 : m_sleepMillis * 1000 };
	const fd_array &sockets = m_readSockets;
	fd_array readSockets = m_readSockets;

	const int ret = select(m_maxFd, &readSockets, NULL, NULL, &t);
	if (0 == ret)
		return true; // nothing
	if (SOCKET_ERROR == ret)
...
bool cTcpServer::AddSession(const SOCKET sock, const Str16 &ip, const int port)
{
	...
	m_sockets.fd_count = 0;
	for (auto &session : m_sessions.m_seq)
		m_sockets.fd_array[m_sockets.fd_count++] = session->m_socket;

    m_maxFd = 0;
	for (auto &session : m_sessions.m_seq)
	{
		if (m_maxFd < session->m_socket)
			m_maxFd = session->m_socket;
	}	
...
unsigned cTcpServer::ThreadFunction(cTcpServer* server)
{
