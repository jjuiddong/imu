...
// template<class T>
// static void MarshallingBinGetSeq(cPacket &packet, T &seq)
// {
// 	int size = 0;
// 	packet >> size;
// 	if (size < 0)
...
// template<class T>
// static void MarshallingBinGetSeq2(cPacket &packet, T &seq)
// {
// 	int size = 0;
// 	packet >> size;
// 	if (size < 0)
...
#define MARSHALLING_BIN_GET_SEQ(packet, SequenceType, seq)\
		{\
			...
			if (size > 10000)\
				return packet;\
			typedef typename SequenceType::value_type type;\
			seq.reserve(size);\
			for (int i = 0; i < size; ++i)\
			{\
				type t;\
