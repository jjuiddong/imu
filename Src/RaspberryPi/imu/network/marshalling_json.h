...
namespace network2
{
	...
	namespace marshalling_json {
		...
		inline cPacket& operator<<(cPacket &packet, const string &rhs)
		{
			packet.AppendPtr(rhs.c_str(), rhs.size());
			return packet;
		}

		inline ptree& put(ptree &props, const char *typeName, const bool rhs) {
			return props.put(typeName, rhs);
		}
		inline ptree& put(ptree &props, const char *typeName, const char rhs) {
			return props.put(typeName, rhs);
		}
		inline ptree& put(ptree &props, const char *typeName, const unsigned char rhs) {
			return props.put(typeName, rhs);
		}
		inline ptree& put(ptree &props, const char *typeName, const short rhs) {
			return props.put(typeName, rhs);
		}
		inline ptree& put(ptree &props, const char *typeName, const unsigned short rhs) {
			return props.put(typeName, rhs);
		}
		inline ptree& put(ptree &props, const char *typeName, const int rhs) {
			return props.put(typeName, rhs);
		}
		...
		inline ptree& put(ptree &props, const char *typeName, const double rhs) {
			return props.put(typeName, rhs);
		}
		inline ptree& put(ptree &props, const char *typeName, const int64 rhs) {
			return props.put(typeName, rhs);
		}
		inline ptree& put(ptree &props, const char *typeName, const uint64 rhs) {
			return props.put(typeName, rhs);
		}
		inline ptree& put(ptree &props, const char *typeName, const string &rhs) {
			return props.put(typeName, rhs);
		}
		inline ptree& put(ptree &props, const char *typeName, const Vector3 &rhs) {
		...
		inline ptree& put(ptree &props, const char *typeName, const T(&rhs)[N])
		{
			for (int i = 0; i < N; ++i)
				put(props, "", rhs[i]);
			return props;
		...
		template<class T>
		inline ptree& put(ptree &props, const char *typeName, const vector<T> &v)
		{
			ptree children;
			for (auto &val : v)
			{
		...
		template<class T>
		inline ptree& put(ptree &props, const char *typeName, const list<T> &v)
		{
			return props;
		}
		...
		//-----------------------------------------------------------------------
		inline cPacket& operator>>(cPacket &packet, OUT string &rhs)
		{
			packet.GetDataString(rhs);
			return packet;
		}

		inline ptree& get(ptree &props, const char *typeName, OUT bool &out) {
			out = props.get<bool>(typeName, false);
			return props;
		}
		inline ptree& get(ptree &props, const char *typeName, OUT char &out) {
			out = props.get<char>(typeName, ' ');
			return props;
		}
		inline ptree& get(ptree &props, const char *typeName, OUT unsigned char &out) {
			out = props.get<unsigned char>(typeName, 0);
			return props;
		}
		inline ptree& get(ptree &props, const char *typeName, OUT short &out) {
			out = props.get<short>(typeName, 0);
			return props;
		}
		inline ptree& get(ptree &props, const char *typeName, OUT unsigned short &out) {
			out = props.get<unsigned short>(typeName, 0);
			return props;
		}
		inline ptree& get(ptree &props, const char *typeName, OUT int &out) {
			out = props.get<int>(typeName, 0);
			return props;
		}
		inline ptree& get(ptree &props, const char *typeName, OUT unsigned int &out) {
		...
		inline ptree& get(ptree &props, const char *typeName, OUT double &out) {
			out = props.get<double>(typeName, 0.f);
			return props;
		}
		inline ptree& get(ptree &props, const char *typeName, OUT int64 &out) {
			out = props.get<int64>(typeName, 0);
			return props;
		}
		inline ptree& get(ptree &props, const char *typeName, OUT uint64 &out) {
			out = props.get<uint64>(typeName, 0);
			return props;
		}
		inline ptree& get(ptree &props, const char *typeName, OUT string &out) {
			out = props.get<string>(typeName, "");
			return props;
		}
		inline ptree& get(ptree &props, const char *typeName, OUT Vector3 &out) {
	...
	// 	inline ptree& get(ptree &props, const char *typeName, OUT _variant_t &out) {
	// 		switch (out.vt)
	// 		{
	...
	// 	}
		...
		template<class T, size_t N>
		inline ptree& get(ptree &props, const char *typeName, OUT T(&out)[N]) {
			for (int i = 0; i < N; ++i)
				get(props, typeName, out[i]);
			return props;
		}
		template<class T>
		inline ptree& get(ptree &props, const char *typeName, OUT vector<T> &out) {
		...
		template<class T> 
		inline ptree& get(ptree &props, const char *typeName, OUT list<T> &out) {
			return props;
		}
